    ...
    <div className="animate-in fade-in duration-200 space-y-6 font-sans">
      <div className="flex flex-col sm:flex-row gap-3 items-center justify-between p-4 rounded-2xl bg-zinc-50/50 dark:bg-zinc-950/20 border border-zinc-200/50 dark:border-zinc-800/60 shadow-xs">
        <div className="relative w-full sm:max-w-xs">
          <input
            type="text"
            placeholder={t('reader.search_words', 'Search words...')}
        ...
        <div className="flex items-center gap-2 shrink-0">
          <span className="text-[10px] font-black text-zinc-400 dark:text-zinc-500 uppercase tracking-wider">{t('reader.sort_label', 'Sort:')}</span>
          <select
          ...
          >
            <option value="alpha">{t('reader.sort_alpha', 'Alphabetical')}</option>
            <option value="appearance">{t('reader.sort_appearance', 'By appearance')}</option>
          </select>
        </div>
      </div>
      ...
      {filteredUnknownWords.length === 0 ? (
        <div className="text-center py-16 bg-zinc-50/50 dark:bg-zinc-900/10 rounded-2xl border border-dashed border-zinc-200 dark:border-zinc-800">
          <p className="text-zinc-500 dark:text-zinc-400 text-sm font-medium">{t('reader.no_unknown_words', 'No unknown words found.')}</p>
        </div>
      ) : (
              ...
              >
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    {translation ? (
                      ...
                      <p className="text-xs text-zinc-400 dark:text-zinc-650 mt-1 italic">
                        {t('reader.no_translation', 'No translation')}
                      </p>
                    )}
                  </div>
                ...
                <div className="flex items-center justify-between border-t border-zinc-100 dark:border-zinc-800/50 pt-3 mt-1 shrink-0 font-sans">
                  <button
                    ...
                    title={t('reader.listen_pronunciation', 'Listen to pronunciation')}
                  >
                  ...
                  <button
                    ...
                    title={t('reader.mark_known', 'Mark as known')}
                  >
                    <Check className="w-3.5 h-3.5" />
                    <span>{t('reader.known', 'Known')}</span>
                  </button>
                </div>
              </div>
            );
