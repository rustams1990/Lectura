...
import { Lesson, LessonType, LingQ } from "./types";
...
// LingQs (Words) Operations
export async function saveLingQ(userId: string, word: string, lingq: LingQ) {
  if (!db) {
    console.warn("Firestore database 'db' is undefined. Skipping saveLingQ.");
    return;
  }
  ...
  try {
    await setDoc(doc(db, "users", userId, "lingqs", normalizedId), {
      ...lingq,
      word: lingq.word ? lingq.word.replace(/^[a-zA-Z]+_/, "") : word.replace(/^[a-zA-Z]+_/, "")
    });
  } catch (err) {
    handleFirestoreError(err, OperationType.WRITE, path);
  }
...
export async function deleteLingQ(userId: string, word: string) {
  if (!db) {
    console.warn("Firestore database 'db' is undefined. Skipping deleteLingQ.");
    return;
  }
...
export async function deleteMultipleLingQs(userId: string, words: string[]) {
  if (!db) {
    console.warn("Firestore database 'db' is undefined. Skipping deleteMultipleLingQs.");
    return;
  }
...
export async function loadUserData(userId: string) {
  ...
  try {
    ...
    lingqsSnap.forEach(doc => {
      ...
      infoLoad.lingqs[decodeDocId(doc.id)] = data;
    });
...
export async function uploadLocalToCloud(
  ...
  // Add all custom words (lingqs)
  for (const word of Object.keys(lingqsMap)) {
    const normId = safeDocId(word);
    const lingqRef = doc(db, "users", userId, "lingqs", normId);
    batch.set(lingqRef, {
      ...lingqsMap[word],
      word: (lingqsMap[word].word || word).replace(/^[a-zA-Z]+_/, "")
    });
    await commitBatchIfNeeded();
  }
