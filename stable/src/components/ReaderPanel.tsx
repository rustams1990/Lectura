...
import { Lesson, LingQ, WordStatus, ReaderSettings } from "../types";
...
export default function ReaderPanel({
  ...
  const [currentPageIdx, setCurrentPageIdx] = useState<number>(() => {
    try {
      const saved = localStorage.getItem(`lingq_progress_${lesson.id}`);
      if (saved) {
  ...
  useEffect(() => {
    if (lesson.id && pages.length > 0) {
      ...
      localStorage.setItem(`lingq_progress_${lesson.id}`, valid.toString());
    }
  ...
  const activePhrasesInLesson = useMemo(() => {
    ...
    return Object.keys(lingqs)
      ...
      .map((key) => lingqs[key]);
  }, [lingqs, textForSearch, lesson.targetLanguage]);
  ...
  const getWordInfo = (cleanWord: string) => {
    ...
    const lq = lingqs[langKey] || lingqs[key];
    if (lq) {
      return lq.status;
    }
  ...
  return (
    <div id="reader-top" className={`relative rounded-2xl border shadow-sm p-6 sm:p-8 space-y-6 transition-colors duration-200 ${currentTheme}`}>
      ...
      >
        {activeSegmentsForPage.map((seg, pIdx) => {
          ...
          const renderSentenceTokens = (sentText: string, sIdx: number) => {
            ...
            const phraseMatches: { phrase: string; lingq: LingQ; tokenIndices: number[] }[] = [];
            ...
            while (wIdx < wordTokens.length) {
              ...
              for (let len = Math.min(10, wordTokens.length - wIdx); len >= 2; len--) {
                ...
                const lq = lingqs[langKey] || lingqs[candidatePhrase];
                if (lq) {
            ...
            const handlePhraseMouseEnter = (
              ...
              const posY = showBelow ? rect.bottom + 12 : rect.top - 12;
              ...
              if (phraseMatch) {
                setHoveredWordObj({
                  ...
                  phraseTranslation: phraseMatch.lingq.translation,
                  phraseStatus: phraseMatch.lingq.status,
                  phraseTags: phraseMatch.lingq.tags,
                });
            ...
            while (tIdx < tokens.length) {
              ...
              if (matchedPhrase) {
                ...
                const status = matchedPhrase.lingq.status;
                let styleClass = "";
                ...
                elements.push(
                  <span key={tIdx} className="inline-block" spellCheck={false}>
                    ...
                    <button
                      ...
                      onMouseEnter={(e) => {
                        ...
                        if (seg.timestamp) {
                          ...
                          setHoveredTimestamp(seg.timestamp);
                ...
                <span key={tIdx} className="inline-block" spellCheck={false}>
                  ...
                  <button
                    ...
                    onMouseEnter={(e) => {
                      ...
                      if (lq) {
                        const rawBaseWord = wordLinks[`${lang}_${cleanWord.toLowerCase()}`] || wordLinks[cleanWord.toLowerCase()];
          ...
          <div className="pt-4 border-t border-zinc-150 dark:border-zinc-800/80 mt-4 space-y-2">
            ...
            <div className="flex flex-wrap gap-1.5 animate-in fade-in duration-200">
              {Object.entries(lesson.detectedPhrases).map(([phrase, details]) => {
                ...
                const lang = lesson.targetLanguage.toLowerCase();
                const langKey = `${lang}_${key}`;
                const isSaved = !!(lingqs[langKey] || lingqs[key]);
      ...
      {hoveredTimestamp && tooltipPos && createPortal(
        <div
          className="fixed z-[9999] pointer-events-none transform -translate-x-1/2 -translate-y-full px-2.5 py-1 bg-zinc-950 dark:bg-zinc-900 border border-zinc-850 dark:border-zinc-750 text-white rounded-lg shadow-xl text-[10px] font-mono font-black tracking-tight flex items-center gap-1 animate-in fade-in zoom-in-95 duration-100"
          style={{ left: `${tooltipPos.x}px`, top: `${tooltipPos.y}px` }}
        >
          <span className="text-teal-400">⏱️</span>
          <span>{hoveredTimestamp}</span>
        </div>,
        document.body
      )}

      {hoveredWordObj && createPortal(
        <div
          className={`fixed z-[9999] pointer-events-none transform -translate-x-1/2 p-3 bg-white dark:bg-zinc-955 border border-zinc-200/90 dark:border-zinc-805 text-zinc-850 dark:text-zinc-100 rounded-xl shadow-xl max-w-xs min-w-[220px] flex flex-col gap-1.5 animate-in fade-in zoom-in-95 duration-100 text-left ${
            hoveredWordObj.position === "below" ? "translate-y-0" : "-translate-y-full"
          }`}
          style={{ left: `${hoveredWordObj.x}px`, top: `${hoveredWordObj.y}px` }}
        >
