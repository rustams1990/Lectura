...
import { LingQ, WordStatus, ExampleSentence, Dictionary, ReaderSettings, Lesson } from "../types";
...
interface WordExplainerProps {
  ...
  existingLingQ?: LingQ | null;
  wordLinks: Record<string, string>;
  lingqs?: Record<string, LingQ> | null;
  onSaveLingQ: (lingq: LingQ) => void;
  onDeleteLingQ: (word: string) => void;
...
export default function WordExplainer({
  ...
  const [popularMeaningsOpen, setPopularMeaningsOpen] = useState(true);
  ...
  const handleSelectImage = (url: string | null) => {
    ...
    if (word) {
      ...
      const updatedLingQ: LingQ = {
        ...
        createdAt: existingLingQ ? existingLingQ.createdAt : Date.now(),
        tags: selectedTags,
        imageUrl: url,
      };
      onSaveLingQ(updatedLingQ);
      if (status === "new") {
        setStatus("2");
      }
    }
  ...
  function HTML_SELECTOR_INITIAL_VALUE(): "ai" | "google" | "free_dictionary" | "wiktionary" | "hybrid" {
    const saved = localStorage.getItem("lingq_clone_translation_source");
  ...
  const handleSetTranslationSource = (src: "ai" | "google" | "free_dictionary" | "wiktionary" | "hybrid") => {
    ...
    localStorage.setItem("lingq_clone_translation_source", src);
  };
  ...
  const [customTags, setCustomTags] = useState<string[]>(() => {
    const saved = localStorage.getItem("lingq_clone_custom_tags");
    if (saved) {
      try {
  ...
  const activeStorageKey = useMemo(() => {
    return `lingq_clone_dicts_${(targetLanguage || "unknown").toLowerCase()}_${(translationLanguage || "unknown").toLowerCase()}`;
  ...
  const isLinked = word ? !!(wordLinks[`${targetLanguage.toLowerCase()}_${word.toLowerCase()}`] || wordLinks[word.toLowerCase()]) : false;
  const linkedParentRaw = word ? (wordLinks[`${targetLanguage.toLowerCase()}_${word.toLowerCase()}`] || wordLinks[word.toLowerCase()] || "") : "";
  ...
  const searchCandidates = useMemo(() => {
    ...
    if (lingqs) {
      Object.values(lingqs).forEach((item) => {
        if (item && item.word) {
  ...
  }, [lingqs, wordLinks, lessonText, lessons]);
  ...
  // Sync with selected word or existing LingQs
  useEffect(() => {
    if (!word) {
      setError(null);
      return;
    ...
    if (existingLingQ) {
      setTranslationValue(existingLingQ.translation);
      setIpaValue(existingLingQ.ipa);
      const cleanGrammar = sanitizeGrammarTag(existingLingQ.grammar);
      setGrammarValue(cleanGrammar);
      setContextRelationValue(existingLingQ.contextRelation);
      setCustomAnswer(existingLingQ.contextRelation || "");
      setExamplesValue(existingLingQ.examples);
      ...
      if (internalStatusUpdateRef.current) {
        ...
        setStatus(existingLingQ.status);
      }
      setSelectedTags(existingLingQ.tags || (cleanGrammar ? [cleanGrammar] : []));
      setImageUrlValue(existingLingQ.imageUrl || null);
    } else {
  ...
  }, [word, existingLingQ, detectedPhrases]);
  ...
  const handleTranslate = async () => {
    ...
    try {
      ...
      setStatus("2"); // start in active learning state level 2
      
      const parsedTags = data.grammar ? [data.grammar] : [];
      setSelectedTags(parsedTags);

      // Create and save LingQ
      const newLingQ: LingQ = {
        word: word.toLowerCase(),
        translation: data.translation || "",
        ipa: data.ipa || "",
        grammar: data.grammar || "",
        contextRelation: data.contextRelation || "",
        status: "2", // default to level 2 learning on auto-translate-creation
        examples: data.examples || [],
        createdAt: Date.now(),
        tags: parsedTags,
        imageUrl: imageUrlValue,
      };
      onSaveLingQ(newLingQ);
    } catch (err: any) {
      console.error(err);
  ...
  const handleSaveCustom = () => {
    ...
    if (!trimmedVal) {
      setStatus("new");
      onDeleteLingQ(word.toLowerCase());
      return;
    }
  ...
  const handleAddCustomTag = () => {
    ...
    if (!customTags.includes(capitalized) && !STANDARD_TAGS.includes(capitalized)) {
      ...
      localStorage.setItem("lingq_clone_custom_tags", JSON.stringify(updated));
    }
  ...
  // Calculate coins based on active status level (represented in LingQ as the yellow coins score award)
  ...
  return (
    <div className={`${themeClasses} rounded-2xl border shadow-md p-3.5 space-y-2.5 flex flex-col h-full transition-all duration-200 max-w-full overflow-hidden select-none`}>
        ...
        <div className="flex items-center gap-2 min-w-0">
          ...
          <div className="min-w-0 font-sans">
            <div className="flex items-center gap-1.5 flex-wrap">
              ...
              {!existingLingQ && detectedPhrases && word && (detectedPhrases[word.toLowerCase()] || detectedPhrases[word]) && (
            ...
            {componentWords.length > 1 && (
              <div className="flex flex-wrap items-center gap-1 mt-1.5 max-w-full">
                {componentWords.map((comp, idx) => {
                  ...
                  const compLingQ = lingqs ? (lingqs[langKey] || lingqs[compKey]) : null;
                  const status = compLingQ ? compLingQ.status : "new";
      ...
      <div className="space-y-2 overflow-y-auto pr-0.5 flex-1 scrollbar-thin dark:dark-scrollbar max-h-[calc(100vh-210px)]">
        ...
        {/* Ask AI Section */}
        <div className="border border-purple-100 dark:border-purple-900/50 rounded-xl overflow-visible bg-purple-50/10 dark:bg-purple-955/5 shrink-0">
          <button
            type="button"
            ...
            <div className="p-2.5 pt-0 border-t border-purple-100/30 dark:border-purple-900/20 space-y-2.5">
              ...
              <div className="flex flex-wrap gap-1">
                ...
                <button
                  ...
                  className="px-2 py-1 bg-purple-55 dark:bg-purple-955/30 hover:bg-purple-100 dark:hover:bg-purple-900/40 text-purple-700 dark:text-purple-300 text-[10px] font-bold rounded-lg border border-purple-100/50 dark:border-purple-900/30 transition-all cursor-pointer"
                >
                  ⚡ Переведи дословно
                </button>
              </div>
        ...
        {/* 3. Popular Meanings Card Wrapper */}
        <div className="border border-zinc-150 dark:border-zinc-800/80 rounded-xl overflow-hidden bg-zinc-50/40 dark:bg-zinc-950/20 font-sans">
          <button
            onClick={() => setPopularMeaningsOpen(!popularMeaningsOpen)}
            className="w-full px-2.5 py-1.5 flex items-center justify-between text-xs font-bold text-zinc-700 dark:text-zinc-300 hover:bg-zinc-100/50 dark:hover:bg-zinc-900/30 transition-colors"
          >
            <span className="uppercase tracking-wider text-[9px] text-zinc-400 dark:text-zinc-500 font-extrabold flex items-center gap-1">
              <Sparkles className="w-3 h-3 text-teal-500 animate-pulse" /> Popular Meanings
            </span>
            {popularMeaningsOpen ? <ChevronUp className="w-3 h-3 text-zinc-400" /> : <ChevronDown className="w-3 h-3 text-zinc-400" />}
          </button>

          {popularMeaningsOpen && (
            <div className="p-2.5 pt-0 border-t border-zinc-100 dark:border-zinc-800 space-y-1 max-h-[170px] overflow-y-auto scrollbar-thin mt-1 w-full">
              ...
              {/* Segmentation Selector block for translation sources */}
              <div className="space-y-1 mt-2.5 pt-1.5 border-t border-zinc-150/40 dark:border-zinc-805/40 text-left shrink-0">
              ...
              {!loading && (
                <button
                  ...
                  className="w-full py-1.5 bg-gradient-to-r from-teal-500 to-teal-600 text-white font-bold text-[10.5px] rounded-lg shadow-3xs hover:shadow-2xs hover:from-teal-600 hover:to-teal-705 transition-all text-center flex items-center justify-center gap-1 cursor-pointer mt-1.5"
                >
        ...
        {/* 4.5. Context search across all uploaded texts */}
        {word && contextSearchHits.length > 0 && (
