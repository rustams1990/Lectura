...
export default function ImportLessonForm({
  ...
  const [targetLanguage, setTargetLanguage] = useState(
    editingLesson?.targetLanguage || localStorage.getItem("lingq_default_target_language") || "Spanish"
  );
  const [translationLanguage, setTranslationLanguage] = useState(
    editingLesson?.translationLanguage || localStorage.getItem("lingq_default_translation_language") || "Russian"
  );

  const [savedTargetLang, setSavedTargetLang] = useState(
    localStorage.getItem("lingq_default_target_language") || "Spanish"
  );
  const [savedTranslationLang, setSavedTranslationLang] = useState(
    localStorage.getItem("lingq_default_translation_language") || "Russian"
  );
  ...
  const handleRememberTargetLanguage = () => {
    localStorage.setItem("lingq_default_target_language", targetLanguage);
    setSavedTargetLang(targetLanguage);
  };

  const handleRememberTranslationLanguage = () => {
    localStorage.setItem("lingq_default_translation_language", translationLanguage);
    ...
    >
      ...
      <div className="space-y-3 bg-zinc-50 dark:bg-zinc-950 p-4 rounded-2xl border border-zinc-100 dark:border-zinc-805">
        <div className="flex items-center justify-between">
          ...
          <button
            type="button"
            onClick={() => setShowTypeCreator(!showTypeCreator)}
          ...
          <div className="p-3.5 bg-white dark:bg-zinc-900 border border-teal-150 dark:border-teal-900/10 rounded-xl space-y-3 animate-in fade-in slide-in-from-top-2 duration-150 border-dashed">
            ...
            <div className="flex justify-end pt-1">
              <button
                ...
                className="px-4 py-2 bg-teal-605 hover:bg-teal-700 text-white font-heavy text-[10px] uppercase tracking-wider rounded-xl shadow-xs cursor-pointer active:scale-97 transition-all"
              >
                Создать и выбрать
              </button>
            </div>
            ...
            return (
              <button
                ...
                className={`px-3 py-2 rounded-xl text-xs font-bold transition-all flex items-center gap-1.5 cursor-pointer border ${
                  isSelected
                    ? "bg-teal-50 text-teal-700 border-teal-200 dark:bg-teal-950/40 dark:text-teal-350 dark:border-teal-900 shadow-xs"
                    : "bg-white hover:bg-zinc-50 border-zinc-200 dark:bg-zinc-900 dark:hover:bg-zinc-805 dark:border-zinc-800 text-zinc-600 dark:text-zinc-400"
                }`}
              >
