...
import { LingQ, Lesson, WordStatus } from "../types";
...
interface StatisticsPageProps {
  lingqs: Record<string, LingQ>;
  lessons: Lesson[];
  listeningSeconds: number;
  onUpdateStatus: (word: string, status: WordStatus, lang?: string) => void;
  onDeleteLingQ: (word: string, lang?: string) => void;
  onDeleteMultipleLingQs?: (words: string[], lang?: string) => void;
  onSaveLingQ?: (newLingQ: LingQ, lang?: string) => void;
  onRenameLingQ?: (oldWord: string, newLingQ: LingQ, lang?: string) => void;
...
export default function StatisticsPage({
  ...
  const handleExecuteBatchImport = () => {
    ...
    parsedBatchWords.forEach((item, index) => {
      ...
      onSaveLingQ(newLingQ, selectedStatsLang);
      importCount++;
    });
  ...
  const startEditingWord = (item: LingQ) => {
  ...
  const handleSaveEditedWord = (item: LingQ) => {
    ...
    if (onRenameLingQ) {
      onRenameLingQ(item.word, updatedLingQ, selectedStatsLang);
    } else {
      if (item.word !== cleanWord) {
        onDeleteLingQ(item.word, selectedStatsLang);
      }
      onSaveLingQ?.(updatedLingQ, selectedStatsLang);
    }
    setEditingWord(null);
  };
    ...
    askConfirm(
      ...
      () => {
        ...
        processedVocabularyList.forEach((item, index) => {
          ...
          onSaveLingQ?.(updated, selectedStatsLang);
        });
  ...
  const handleDeleteAllLanguageWords = () => {
    if (lingqArray.length === 0) return;
    
    askConfirm(
      "ПОЛНОЕ УДАЛЕНИЕ СЛОВАРЯ",
      `Вы собираетесь ПОЛНОСТЬЮ удалить ВСЕ слова для языка "${selectedStatsLang}" (${lingqArray.length} шт.).`,
      "ВНИМАНИЕ! Это действие абсолютно необратимо и сотрет ваш словарь для этого языка. Вы точно хотите начать заново?",
      () => {
        if (onDeleteMultipleLingQs) {
          const words = lingqArray.map((item) => item.word);
          onDeleteMultipleLingQs(words, selectedStatsLang);
        } else {
          // Loop fallback
  ...
  // Find all languages dynamically that have at least one word (LingQ) entry
  ...
  const lingqArray = useMemo(() => {
    return Object.entries(lingqs)
      .filter(([key, lq]) => {
        const parts = key.split("_");
        const itemLang = parts.length > 1 ? parts[0] : "spanish";
  ...
  }, [lingqArray, onlyPatterns, selectedStatsLang, wordLinks]);
  ...
  const cefrProfileCounts = useMemo(() => {
    ...
    statsArray.forEach((item) => {
      ...
      const uppercaseTags = tags.map(t => t.toUpperCase().trim());
  ...
  const handleAutoProfileVocabulary = () => {
    ...
    setTimeout(() => {
      try {
        ...
        lingqArray.forEach((lingqItem) => {
          const w = lingqItem.word.toLowerCase();
          ...
          const originalTags = lingqItem.tags || [];
  ...
  const filteredVocabularyList = useMemo(() => {
    const list = statsArray.filter((item) => {
      const matchesSearch = 
        ...
        (item.grammar && item.grammar.toLowerCase().includes(vocabSearch.toLowerCase()));
      ...
      const matchesFilter = (() => {
        ...
        if (vocabFilter === "learning") {
          return ["1", "2", "3", "4", "5", "learning"].includes(item.status);
        }
      ...
      const matchesTag = (() => {
        ...
        return !!(item.tags && Array.isArray(item.tags) && item.tags.some(t => t.toLowerCase() === vocabTagFilter.toLowerCase()));
      })();
  ...
  const processedVocabularyList = useMemo(() => {
    ...
    const grouped = new Map<string, LingQ>();
        ...
        <div id="stats-lang-selector" className="bg-white dark:bg-zinc-900 border border-zinc-150 dark:border-zinc-805 p-3 rounded-2xl shadow-xs font-sans">
          ...
          <div className="flex flex-wrap gap-2">
            {languagesList.map((lang) => {
              const wordsCount = Object.entries(lingqs).filter(([key, lq]) => {
                const parts = key.split("_");
                const itemLang = parts.length > 1 ? parts[0] : "spanish";
                return itemLang.toLowerCase() === lang.toLowerCase();
              }).length;
            ...
            <div className="py-4 flex-1 flex flex-col justify-end space-y-4">
              ...
              <div className="bg-zinc-50 dark:bg-zinc-955 p-3 rounded-lg border border-zinc-150/50 dark:border-zinc-805/50 space-y-1 text-[11px] text-zinc-650 dark:text-zinc-300">
                <div className="flex justify-between font-medium select-none">
                  ...
                  <strong className="text-zinc-900 dark:text-white">{statsArray.length} {onlyPatterns ? "паттернов" : "слов"}</strong>
                </div>
                <div className="flex justify-between font-medium select-none text-[10.5px]">
                  <span>Позапрошлый месяц:</span>
                  <span className="text-zinc-500">{monthlyGrowth[3]?.count || 0} {onlyPatterns ? "паттернов" : "слов"}</span>
                </div>
                <div className="flex justify-between font-medium select-none text-[10.5px]">
      ...
      <div className="bg-white dark:bg-zinc-900 border border-zinc-150 dark:border-zinc-800 p-5 rounded-2xl shadow-xs space-y-5 font-sans">
        ...
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 border-b border-zinc-100 dark:border-zinc-805 pb-3">
          ...
          <button
            ...
            disabled={isProfiling || lingqArray.length === 0}
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between text-xs select-none">
            ...
            <span className="font-mono text-zinc-400">
              Размечено: <strong className="text-zinc-700 dark:text-zinc-200 font-extrabold">{cefrProfileCounts.totalWithCefr}</strong> из <strong className="text-zinc-700 dark:text-zinc-200 font-extrabold">{lingqArray.length}</strong> слов
            </span>
          </div>
      ...
      <div className="bg-white dark:bg-zinc-900 border border-zinc-150 dark:border-zinc-800 p-5 rounded-2xl shadow-xs space-y-4">
        ...
        <div className="border-b border-zinc-100 dark:border-zinc-805 pb-3 flex flex-col sm:flex-row items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2 shrink-0 select-none">
            ...
            <div className="flex bg-zinc-100 dark:bg-zinc-950 px-3 py-1.5 rounded-xl border border-zinc-200/50 dark:border-zinc-805/50 items-center gap-1.5 header-badge-layout select-none">
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-3">
          ...
          <div className="relative">
            ...
            >
              <option value="all">🌐 Все активные ${onlyPatterns ? "паттерны" : "слова"} ({statsArray.filter(l => l.status !== "ignored").length})</option>
                ...
                <div className="md:col-span-7 space-y-1.5">
                  ...
                  {parsedBatchWords.length > 0 && (
                    <div className="text-[10px] text-zinc-500 dark:text-zinc-400 font-bold bg-teal-500/5 dark:bg-teal-400/5 px-2.5 py-1 rounded-lg border border-teal-500/10 flex items-center justify-between select-none animate-pulse">
                      <span>🔍 Распознано слов к импорту: <strong className="text-teal-600 dark:text-teal-450">{parsedBatchWords.length} шт.</strong></span>
            ...
            <div className="bg-red-500/5 dark:bg-red-500/15 p-4 rounded-xl border border-red-200/40 dark:border-red-900/30 space-y-3 mt-1.5 font-sans">
              ...
              <div className="flex flex-wrap items-center gap-2 pt-1">
                ...
                >
                  ...
                  <span>Удалить все слова {selectedStatsLang.toUpperCase()} ({lingqArray.length} шт.)</span>
                </button>

                {processedVocabularyList.length > 0 && processedVocabularyList.length < lingqArray.length && (
                  <button
                    type="button"
                      ...
                      <td className="p-3 text-center text-[11px] font-mono text-zinc-500 dark:text-zinc-400">
                        {editingWordDate === item.word ? (
                          <div className="flex items-center justify-center gap-1">
                            <input
                              type="date"
                              defaultValue={item.createdAt ? new Date(item.createdAt).toISOString().split('T')[0] : ""}
                              onChange={(e) => {
                              ...
                              className="bg-white dark:bg-zinc-950 border border-zinc-250 dark:border-zinc-800 rounded px-1.5 py-1 text-xs font-mono text-zinc-850 dark:text-zinc-150 focus:outline-none focus:ring-1 focus:ring-teal-500"
                              autoFocus
                            />
                            <button
                              type="button"
                      ...
                      <td className="p-3 text-center">
                        {editingWord === item.word ? (
                          ...
                          <div className="flex items-center justify-center gap-1">
                            ...
                            <button
                              type="button"
                              onClick={() => onDeleteLingQ(item.word, selectedStatsLang)}
