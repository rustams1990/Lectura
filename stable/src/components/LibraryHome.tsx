...
import { Lesson, LessonType, AppStats } from "../types";
...
import { ICON_MAP } from "./ImportLessonForm";
...
export default function LibraryHome({
  ...
  const [dailyGoal, setDailyGoal] = useState<number>(() => {
    const saved = localStorage.getItem("lingq_clone_daily_word_goal");
  ...
  const LANGUAGE_TIPS = [
    ...
    "Метод LingQ: не бойтесь новых слов! Ваша цель — перевести их в статус 'изучаемых' и читать дальше.",
  ...
  const handleCycleGoal = (e: React.MouseEvent) => {
    ...
    localStorage.setItem("lingq_clone_daily_word_goal", nextGoal.toString());
  };
  ...
  const lastOpenedLessonId = useMemo(() => {
    return localStorage.getItem("lingq_clone_last_active_lesson_id") || null;
  }, []);
  ...
  const resumeLessonStats = useMemo(() => {
    ...
    processedWords.forEach(word => {
      ...
      const item = lingqs[langKeyForResolved] || lingqs[resolvedKey];
  ...
  const todayCreatedCount = useMemo(() => {
    ...
    return Object.values(lingqs).filter((item: any) => {
      return item.createdAt && item.createdAt >= startTs;
    }).length;
  }, [lingqs]);

  const [booksPerRow, setBooksPerRow] = useState<number>(() => {
    const saved = localStorage.getItem("lingq_books_per_row");
    return saved ? parseInt(saved, 10) : 4;
  });

  const handleBooksPerRowChange = (cols: number) => {
    setBooksPerRow(cols);
    localStorage.setItem("lingq_books_per_row", cols.toString());
  };
        ...
        <div className="border-t border-zinc-100 dark:border-zinc-800 pt-3 flex items-center gap-2">
          ...
          <div className="flex flex-wrap gap-1.5 items-center">
            ...
            {lessonTypes.map((type) => {
              const IconComponent = ICON_MAP[type.icon] || ICON_MAP.type;
      ...
      {filteredLessons.length > 0 ? (
        <>
          <div className={`grid ${gridColsClass} gap-6 md:gap-8`}>
            ...
            // Calculate exact known and learning word percentages for understanding indicators
            const bookStats = (() => {
              ...
              if (processedWords.length === 0) {
                return { knownPct: 0, unknownPct: 100, knownCount: 0, unknownCount: 0, total: 0 };
              }
            ...
            return (
              ...
              >
                  ...
                  <div className="flex items-center justify-between z-10 w-full animate-in fade-in duration-300">
                    ...
                    <div className="flex items-center gap-1.5">
                      ...
                      {(() => {
                        ...
                        const IconComponent = ICON_MAP[typeInfo?.icon || "book"] || ICON_MAP.book;
                        return (
                ...
                <div className="p-4 flex-1 flex flex-col justify-between space-y-4">
                  ...
                  {/* Understanding Metrics Pills and Tooltips */}
                  <div className="space-y-1 bg-zinc-50 dark:bg-zinc-950/20 p-2 rounded-xl border border-zinc-200/50 dark:border-zinc-800/30 select-none">
                    <div className="flex justify-between items-center text-[9px] uppercase font-black tracking-widest text-zinc-400">
                      <span>Понимание (Comprehension)</span>
                      <span className="text-zinc-600 dark:text-zinc-300 font-extrabold">{bookStats.knownPct}%</span>
                    </div>

                    {/* Proportional dual comprehension bar */}
                    <div className="h-1.5 w-full rounded-full bg-sky-500/20 flex overflow-hidden">
                      <div 
                        ...
                        title={`Понимание: ${bookStats.knownPct}% (Известно слов: ${bookStats.knownCount} из ${bookStats.total})`}
                      />
                      <div 
                        style={{ width: `${bookStats.unknownPct}%` }}
                        className="bg-sky-400 h-full transition-all duration-300 cursor-help"
                        title={`Непонимание: ${bookStats.unknownPct}% (Неизвестно слов: ${bookStats.unknownCount} из ${bookStats.total})`}
                      />
                    </div>

                    <div className="flex justify-between items-center text-[9px] font-extrabold font-sans">
                      <span 
                        title={`Количество известных слов во фрагменте: ${bookStats.knownCount}`}
                        className="text-emerald-500 hover:underline cursor-help animate-none"
                      >
                        Понятно: {bookStats.knownPct}% ({bookStats.knownCount} слов)
                      </span>
                      <span 
                        title={`Количество неизвестных или новых слов во фрагменте: ${bookStats.unknownCount}`}
