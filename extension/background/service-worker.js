...
function normalizeLangKey(lang) {
  if (!lang) return "en";
  const l2 = lang.trim().toLowerCase();
  if (l2.startsWith("es") || l2.includes("span")) return "es";
  if (l2.startsWith("en") || l2.includes("engl")) return "en";
  if (l2.startsWith("fr") || l2.includes("fren")) return "fr";
  if (l2.startsWith("de") || l2.includes("germ")) return "de";
  if (l2.startsWith("ru") || l2.includes("russ")) return "ru";
  if (l2.startsWith("it") || l2.includes("ital")) return "it";
  if (l2.startsWith("pt") || l2.includes("port")) return "pt";
  if (l2.startsWith("zh") || l2.includes("chin")) return "zh";
  if (l2.startsWith("ja") || l2.includes("jap")) return "ja";
  if (l2.startsWith("ko") || l2.includes("kore")) return "ko";
  return l2.slice(0, 2);
}
...
function mergeDeep(model5, plugin6) {
  if (isObject(plugin6)) {
    for (const key in plugin6) {
      if (isUnsafeKey(key)) {
        continue;
      }
      if (isObject(plugin6[key])) {
        if (!model5[key]) Object.assign(model5, { [key]: {} });
        mergeDeep(model5[key], plugin6[key]);
      } else {
        Object.assign(model5, { [key]: plugin6[key] });
      }
    }
  }
  return model5;
}
function mergeQuick(model5, plugin6) {
  for (const key in plugin6) {
...
var extend = function(plugin6, world2, View2, nlp2) {
  if (isArray2(plugin6)) {
    plugin6.forEach((p5) => extend(p5, world2, View2, nlp2));
    return;
  }
  const { methods: methods17, model: model5, compute: compute4, hooks: hooks2 } = world2;
  if (plugin6.methods) {
    mergeQuick(methods17, plugin6.methods);
  }
  if (plugin6.model) {
    mergeDeep(model5, plugin6.model);
  }
  if (plugin6.irregulars) {
    addIrregulars(model5, plugin6.irregulars);
  }
  if (plugin6.compute) {
    Object.assign(compute4, plugin6.compute);
  }
  if (hooks2) {
    world2.hooks = hooks2.concat(plugin6.hooks || []);
  }
  if (plugin6.api) {
    plugin6.api(View2);
  }
  if (plugin6.lib) {
    Object.keys(plugin6.lib).forEach((k2) => nlp2[k2] = plugin6.lib[k2]);
  }
  if (plugin6.tags) {
    nlp2.addTags(plugin6.tags);
  }
  if (plugin6.words) {
    nlp2.addWords(plugin6.words);
  }
  if (plugin6.frozen) {
    nlp2.addWords(plugin6.frozen, true);
  }
  if (plugin6.mutate) {
    plugin6.mutate(world2, nlp2);
  }
};
...
nlp.plugin = function(plugin6) {
  extend_default(plugin6, this._world, View_default, this);
  return this;
};
...
// node_modules/compromise/src/1-one/output/api/_spec.js
var attributeTags = /* @__PURE__ */ new Set(["Hyphenated", "Prefix", "SlashedTerm"]);
var rootOf = function(tag, tagSet) {
  const entry = tagSet[tag];
  if (!entry || !entry.parents || entry.parents.length === 0) {
    return tag;
  }
  for (let i3 = 0; i3 < entry.parents.length; i3 += 1) {
    const p5 = entry.parents[i3];
    if (tagSet[p5] && (!tagSet[p5].parents || tagSet[p5].parents.length === 0)) {
      return p5;
    }
  }
  return entry.parents[entry.parents.length - 1];
};
var slotForTerm = function(term, tagSet) {
  const tags = Array.from(term.tags || []);
  if (tags.length === 0) {
    return "-";
  }
  const primary = tags.find((t3) => !attributeTags.has(rootOf(t3, tagSet))) || tags[0];
  return rootOf(primary, tagSet);
};
var makeAliases = function(tagSet) {
  const aliases3 = {};
  for (const tag in tagSet) {
    const entry = tagSet[tag];
    if (entry.alias) {
      aliases3[tag] = entry.alias;
    }
  }
  return aliases3;
};
var toSpec = function(doc, world2) {
  const tagSet = world2.model.one.tagSet;
  const aliases3 = makeAliases(tagSet);
  return doc.docs.map((terms) => {
    const text = terms.reduce((str, t3) => str + t3.pre + t3.text + t3.post, "").trim();
    const tags = terms.map((t3) => {
      let tag = slotForTerm(t3, tagSet);
      return aliases3[tag] || tag;
    }).join(",");
    return `${text} {${tags}}`;
  }).join("\n");
};
var spec_default = toSpec;
...
var out = function(method) {
  ...
  if (method === "spec") {
    return spec_default(this, this.world);
  }
  if (method === "json") {
    return this.json();
  }
...
// node_modules/compromise/src/1-one/output/fromSpec.js
var lastBrace = /\{(?=[^{]*$)/;
var parseLine = function(line = "") {
  ...
  tags = tags.map((tag) => tag.split("|").map((t3) => t3.trim()));
  tags = tags.filter((arr) => arr.some((t3) => t3 !== ""));
  return { text, tags };
};
var toMatchString = function(tags, aliases3) {
  return tags.map((arr) => {
    arr = arr.map((str) => {
      return "#" + (aliases3[str] || str);
    });
    if (arr.length > 1) {
      return `(${arr.join(" && ")})`;
    }
...
var testSpec = function(spec, verbose2 = true, throwError = false) {
  let world2 = this.world();
  let aliases3 = {};
  let tagSet = world2.model.one.tagSet;
  Object.keys(tagSet).forEach((k2) => {
    if (tagSet[k2].alias) {
      aliases3[tagSet[k2].alias] = k2;
    }
  });
  let failingLines = spec.split("\n").filter((line) => line.trim()).map((line) => {
    let { text, tags } = parseLine(line);
    let doc = this(text);
    let matchStr = toMatchString(tags, aliases3);
    let didMatch = doc.has(matchStr);
    if (verbose2 !== false) {
      let char = didMatch ? "\u2705" : "\u274C";
      console.log(`${char} ${text} {${toTagList(tags)}}`);
    }
...
var compute3 = function(allTags2) {
  const flatList = Object.keys(allTags2).map((k2) => {
    ...
    return { id: k2, parent: o2.is, props, children: [], alias: o2.alias };
  });
...
var chillin = /[sn]['’]$/;
var isFullNumber = /^[(+\-]?\d+(th|st|nd|rd)?[)+\-]?$/;
var normalizePunctuation = function(str, model5) {
  ...
  for (let i3 = 0; i3 < len; i3 += 1) {
    ...
    if ((c2 === "+" || c2 === "-" || c2 === "(") && isFullNumber.test(str.trim())) {
      break;
    }
  ...
  for (let i3 = 0; i3 < len; i3 += 1) {
    ...
    if ((c2 === "+" || c2 === ")") && isFullNumber.test(str.trim())) {
      break;
    }
    post = chars.pop() + post;
  }
...
var addresses = function(n3) {
  let m3 = this.match("#Address+");
  m3 = m3.getNth(n3);
  return m3;
};
var selections = [
...
var addMethods = function(View2) {
  ...
  View2.prototype.addresses = addresses;
  aliases2.forEach((a2) => {
...
var plugin3 = function(View2) {
  class Fractions extends View2 {
    ...
    toText(n3) {
      this.getNth(n3).forEach((m3) => {
        const obj = parse_default4(m3);
        const str = toOrdinal_default(obj);
        if (str) {
          m3.replaceWith(str);
        }
      });
...
var addMethod = function(View2) {
  ...
  View2.prototype.percentages = function(n3) {
    let m3 = find_default3(this);
    m3 = m3.filter((v2) => v2.has("#Percent"));
    m3 = m3.getNth(n3);
    return new Numbers(this.document, m3.pointer);
  };
...
// node_modules/compromise/src/3-three/numbers/money/currencies.js
...
// node_modules/compromise/src/3-three/numbers/money/api.js
var find5 = function(doc) {
  return doc.match("#Money+ #Currency? (#Money+ #Currency?)?");
};
var parse5 = function(m3) {
  m3 = m3.clone();
  let currency = m3.match("#Currency").nouns().toSingular().text("normal");
  const num = m3.match("#Money").numbers().get()[0];
  if (!currency) {
    let str = m3.text();
    const found = currencies_default.find(([sym]) => str.includes(sym));
    if (found) {
      currency = found[1];
    }
  }
...
var plugin4 = function(View2) {
  class Money extends View2 {
    constructor(document, pointer, groups) {
      super(document, pointer, groups);
      this.viewType = "Money";
    }
    parse(n3) {
      return this.getNth(n3).map(parse5);
    }
    get(n3) {
      return this.getNth(n3).map(parse5).map((p5) => p5.num);
    }
    json(n3) {
      return this.getNth(n3).map((p5) => {
        const json = p5.toView().json(n3)[0];
        const parsed = parse5(p5);
        json.money = parsed;
        return json;
      }, []);
    }
  ...
  View2.prototype.money = function(n3) {
    let m3 = find5(this);
    m3 = m3.getNth(n3);
    return new Money(this.document, m3.pointer);
  };
};
var api_default17 = plugin4;
...
// node_modules/compromise/src/3-three/redact/redact.js
var defaults3 = {
  people: true,
  places: true,
  organizations: true,
...
var redactMatch = function(m3, blockStr, keep9 = true) {
  m3 = m3.notIf("#Redacted");
  m3.replaceWith(blockStr, keep9);
  m3.tag("Redacted");
  return m3;
};
var redact = function(opts2 = {}, blockStr = "\u2588\u2588\u2588\u2588\u2588\u2588\u2588\u2588\u2588\u2588", keep9 = true) {
  opts2 = Object.assign({}, defaults3, opts2);
  if (opts2.people !== false) {
    redactMatch(this.people(), blockStr, keep9).unTag(hideTags.people);
  }
  if (opts2.places !== false) {
    redactMatch(this.places(), blockStr, keep9).unTag(hideTags.places);
  }
  if (opts2.organizations !== false) {
    redactMatch(this.organizations(), blockStr, keep9).unTag(hideTags.organizations);
  }
  if (opts2.emails !== false) {
    redactMatch(this.emails(), blockStr, keep9);
  }
  if (opts2.money !== false) {
    redactMatch(this.money(), blockStr, keep9);
  }
  if (opts2.percentages !== false) {
    redactMatch(this.percentages(), blockStr, keep9);
  }
  if (opts2.fractions !== false) {
    redactMatch(this.fractions(), blockStr, keep9);
  }
  if (opts2.phoneNumbers !== false) {
    redactMatch(this.phoneNumbers(), blockStr, keep9);
  }
  if (opts2.atMentions !== false) {
    redactMatch(this.atMentions(), blockStr, keep9);
  }
  if (opts2.acronyms !== false) {
    redactMatch(this.acronyms(), blockStr, keep9);
  }
  if (opts2.urls !== false) {
    redactMatch(this.urls(), blockStr, keep9);
  }
  if (opts2.properNouns !== false) {
    redactMatch(this.properNouns(), blockStr, keep9);
  }
  if (opts2.dates !== false) {
    redactMatch(this.dates(), blockStr, keep9);
  }
  if (opts2.numbers !== false) {
    redactMatch(this.numbers(), blockStr, keep9);
  }
  if (opts2.pronouns !== false) {
    redactMatch(this.pronouns(), blockStr, keep9).unTag(hideTags.pronouns);
  }
  return this;
};
var redact_default = redact;
...
var plugin5 = {
  ...
  api: function(View2) {
    View2.prototype.redact = redact_default;
  }
};
var plugin_default26 = plugin5;
...
var api_default18 = api15;

// node_modules/compromise/src/3-three/sentences/plugin.js
var plugin_default27 = { api: api_default18 };
...
// src/data/dictionaries/english_irregulars.json
var english_irregulars_default = {
  went: ["go"],
  gone: ["go"],
  was: ["be"],
  were: ["be"],
...
// src/data/dictionaries/spanish_irregulars.json
var spanish_irregulars_default = {
  fui: ["ser", "ir"],
  fuiste: ["ser", "ir"],
  fue: ["ser", "ir"],
  fuimos: ["ser", "ir"],
...
// src/data/dictionaries/spanish_enclitics.json
var spanish_enclitics_default = {
  double: [
    "melo",
    "mela",
    "melos",
...
// src/data/dictionaries/spanish_short_imperatives.json
var spanish_short_imperatives_default = {
  di: ["decir"],
  haz: ["hacer"],
  ve: ["ir", "ver"],
  pon: ["poner"],
...
// src/data/dictionaries/spanish_future_stems.json
var spanish_future_stems_default = {
  tendr: ["tener"],
  har: ["hacer"],
  dir: ["decir"],
  podr: ["poder"],
...
// src/data/dictionaries/spanish_high_confidence_endings.json
var spanish_high_confidence_endings_default = [
  "aba",
  "abas",
  "aban",
  "\xE1bamos",
...
// src/data/dictionaries/spanish_accented_verbs.json
var spanish_accented_verbs_default = {
  sonreir: "sonre\xEDr",
  reir: "re\xEDr",
  freir: "fre\xEDr",
  oir: "o\xEDr"
...
// src/data/dictionaries/spanish_verb_endings.json
var spanish_verb_endings_default = [
  { ending: "i\xE9semos", infinitives: ["er", "ir"] },
  { ending: "i\xE9ramos", infinitives: ["er", "ir"] },
  { ending: "ar\xEDamos", infinitives: ["ar"] },
  { ending: "er\xEDamos", infinitives: ["er"] },
  { ending: "ir\xEDamos", infinitives: ["ir"] },
  { ending: "\xE1semos", infinitives: ["ar"] },
  { ending: "\xE1ramos", infinitives: ["ar"] },
  { ending: "asteis", infinitives: ["ar"] },
  { ending: "isteis", infinitives: ["er", "ir"] },
  { ending: "ar\xE9is", infinitives: ["ar"] },
  { ending: "areis", infinitives: ["ar"] },
  { ending: "er\xE9is", infinitives: ["er"] },
  { ending: "ereis", infinitives: ["er"] },
  { ending: "ir\xE9is", infinitives: ["ir"] },
  { ending: "ireis", infinitives: ["ir"] },
  { ending: "arais", infinitives: ["ar"] },
  { ending: "aseis", infinitives: ["ar"] },
  { ending: "erais", infinitives: ["er"] },
  { ending: "eseis", infinitives: ["er"] },
  { ending: "irais", infinitives: ["ir"] },
  { ending: "iseis", infinitives: ["ir"] },
  ...
  { ending: "ar\xEDais", infinitives: ["ar"] },
  { ending: "er\xEDais", infinitives: ["er"] },
  { ending: "ir\xEDais", infinitives: ["ir"] },
  { ending: "\xE1bamos", infinitives: ["ar"] },
  ...
  { ending: "ar\xEDan", infinitives: ["ar"] },
  { ending: "er\xEDan", infinitives: ["er"] },
  { ending: "ir\xEDan", infinitives: ["ir"] },
  { ending: "ar\xEDas", infinitives: ["ar"] },
  { ending: "er\xEDas", infinitives: ["er"] },
  { ending: "ir\xEDas", infinitives: ["ir"] },
  { ending: "abais", infinitives: ["ar"] },
  { ending: "\xEDais", infinitives: ["er", "ir"] },
  { ending: "\xEDamos", infinitives: ["er", "ir"] },
  ...
  { ending: "ar\xE1n", infinitives: ["ar"] },
  { ending: "er\xE1n", infinitives: ["er"] },
  { ending: "ir\xE1n", infinitives: ["ir"] },
  { ending: "ar\xE1s", infinitives: ["ar"] },
  { ending: "er\xE1s", infinitives: ["er"] },
  { ending: "ir\xE1s", infinitives: ["ir"] },
  { ending: "abas", infinitives: ["ar"] },
  { ending: "aban", infinitives: ["ar"] },
  { ending: "\xEDas", infinitives: ["er", "ir"] },
  { ending: "\xEDan", infinitives: ["er", "ir"] },
  { ending: "ar\xEDa", infinitives: ["ar"] },
  { ending: "er\xEDa", infinitives: ["er"] },
  { ending: "ir\xEDa", infinitives: ["ir"] },
  { ending: "ar\xE1", infinitives: ["ar"] },
  { ending: "er\xE1", infinitives: ["er"] },
  { ending: "ir\xE1", infinitives: ["ir"] },
  ...
  { ending: "\xE1is", infinitives: ["ar", "er", "ir"] },
  { ending: "\xE9is", infinitives: ["ar", "er", "ir"] },
  ...
  { ending: "\xEDs", infinitives: ["ir"] },
  { ending: "i\xF3", infinitives: ["er", "ir"] },
  { ending: "ar\xE9", infinitives: ["ar"] },
  { ending: "er\xE9", infinitives: ["er"] },
  { ending: "ir\xE9", infinitives: ["ir"] },
  { ending: "\xF3", infinitives: ["ar"] },
  { ending: "\xE9", infinitives: ["ar"] },
  { ending: "\xED", infinitives: ["er", "ir"] },
...
// src/data/dictionaries/spanish_common_verbs.json
var spanish_common_verbs_default = [
  "abrazar",
  "abrir",
  "acabar",
  "acceder",
...
// src/data/dictionaries/spanish_homonyms.json
var spanish_homonyms_default = {
  _comment: "Homonym control dictionary. 'priority' reorders/filters suggestions for ambiguous verb forms. 'exclude' suppresses verb suggestions for pure function words that accidentally match verb patterns.",
  priority: {
    ve: ["ver", "ir"],
    fue: ["ser", "ir"],
    sea: ["ser"],
...
// src/data/dictionaries/portuguese_irregulars.json
var portuguese_irregulars_default = {
  sou: ["ser"],
  \u00E9s: ["ser"],
  \u00E9: ["ser"],
  somos: ["ser"],
...
// src/data/dictionaries/portuguese_common_verbs.json
var portuguese_common_verbs_default = [
  "abrir",
  "acabar",
  "aceitar",
  "achar",
...
// src/data/dictionaries/french_irregulars.json
var french_irregulars_default = {
  suis: ["\xEAtre", "suivre"],
  es: ["\xEAtre"],
  est: ["\xEAtre"],
  sommes: ["\xEAtre"],
...
// src/data/dictionaries/french_common_verbs.json
var french_common_verbs_default = [
  "accepter",
  "acheter",
  "agir",
  "aider",
...
// src/data/dictionaries/italian_irregulars.json
var italian_irregulars_default = {
  sono: ["essere"],
  sei: ["essere"],
  \u00E8: ["essere"],
  siamo: ["essere"],
...
// src/data/dictionaries/italian_common_verbs.json
var italian_common_verbs_default = [
  "abitare",
  "accettare",
  "accompagnare",
  "aiutare",
...
// src/data/dictionaries/german_irregulars.json
var german_irregulars_default = {
  bin: ["sein"],
  bist: ["sein"],
  ist: ["sein"],
  sind: ["sein"],
...
// src/data/dictionaries/german_common_verbs.json
var german_common_verbs_default = [
  "antworten",
  "arbeiten",
  "bauen",
  "bedeuten",
...
// src/data/dictionaries/russian_irregulars.json
var russian_irregulars_default = {
  \u0435\u0441\u0442\u044C: ["\u0431\u044B\u0442\u044C", "\u0435\u0441\u0442\u044C"],
  \u0431\u044B\u043B: ["\u0431\u044B\u0442\u044C"],
  \u0431\u044B\u043B\u0430: ["\u0431\u044B\u0442\u044C"],
  \u0431\u044B\u043B\u043E: ["\u0431\u044B\u0442\u044C"],
...
// src/data/dictionaries/russian_common_verbs.json
var russian_common_verbs_default = [
  "\u0431\u0435\u0436\u0430\u0442\u044C",
  "\u0431\u0435\u0440\u0435\u0447\u044C",
  "\u0431\u0438\u0442\u044C",
  "\u0431\u043B\u0430\u0433\u043E\u0434\u0430\u0440\u0438\u0442\u044C",
...
// src/data/dictionaries/ukrainian_irregulars.json
var ukrainian_irregulars_default = {
  \u0454: ["\u0431\u0443\u0442\u0438"],
  \u0431\u0443\u0432: ["\u0431\u0443\u0442\u0438"],
  \u0431\u0443\u043B\u0430: ["\u0431\u0443\u0442\u0438"],
  \u0431\u0443\u043B\u043E: ["\u0431\u0443\u0442\u0438"],
  ...
  \u0432\u0437\u044F\u043B\u0438: ["\u0432\u0437\u044F\u0442\u0438"],
  \u043A\u0430\u0436\u0443: ["\u0441\u043A\u0430\u0437\u0430\u0442\u0438", "\u043A\u0430\u0437\u0430\u0442\u0438"],
  \u043A\u0430\u0436\u0435\u0448: ["\u0441\u043A\u0430\u0437\u0430\u0442\u0438", "\u043A\u0430\u0437\u0430\u0442\u0438"],
  \u043A\u0430\u0436\u0435: ["\u0441\u043A\u0430\u0437\u0430\u0442\u0438", "\u043A\u0430\u0437\u0430\u0442\u0438"],
  \u043A\u0430\u0436\u0435\u043C\u043E: ["\u0441\u043A\u0430\u0437\u0430\u0442\u0438", "\u043A\u0430\u0437\u0430\u0442\u0438"],
  \u043A\u0430\u0436\u0435\u0442\u0435: ["\u0441\u043A\u0430\u0437\u0430\u0442\u0438", "\u043A\u0430\u0437\u0430\u0442\u0438"],
  \u043A\u0430\u0436\u0443\u0442\u044C: ["\u0441\u043A\u0430\u0437\u0430\u0442\u0438", "\u043A\u0430\u0437\u0430\u0442\u0438"],
  \u043A\u0430\u0437\u0430\u0432: ["\u0441\u043A\u0430\u0437\u0430\u0442\u0438", "\u043A\u0430\u0437\u0430\u0442\u0438"],
  \u043A\u0430\u0437\u0430\u043B\u0430: ["\u0441\u043A\u0430\u0437\u0430\u0442\u0438", "\u043A\u0430\u0437\u0430\u0442\u0438"],
  \u043A\u0430\u0437\u0430\u043B\u0438: ["\u0441\u043A\u0430\u0437\u0430\u0442\u0438", "\u043A\u0430\u0437\u0430\u0442\u0438"],
  \u0433\u043E\u0432\u043E\u0440\u044E: ["\u0433\u043E\u0432\u043E\u0440\u0438\u0442\u0438"],
  \u0433\u043E\u0432\u043E\u0440\u0438\u0448: ["\u0433\u043E\u0432\u043E\u0440\u0438\u0442\u0438"],
  \u0433\u043E\u0432\u043E\u0440\u0438\u0442\u044C: ["\u0433\u043E\u0432\u043E\u0440\u0438\u0442\u0438"],
  \u0433\u043E\u0432\u043E\u0440\u0438\u043C\u043E: ["\u0433\u043E\u0432\u043E\u0440\u0438\u0442\u0438"],
...
// src/data/dictionaries/ukrainian_common_verbs.json
var ukrainian_common_verbs_default = [
  "\u0431\u0430\u0447\u0438\u0442\u0438",
  "\u0431\u0430\u0447\u0438\u0442\u0438\u0441\u044F",
  "\u0431\u0456\u0433\u0442\u0438",
  "\u0431\u0435\u0440\u0435\u0433\u0442\u0438",
...
// src/data/dictionaries/kazakh_irregulars.json
var kazakh_irregulars_default = {
  \u0431\u043E\u043B: ["\u0431\u043E\u043B\u0443"],
  \u0431\u043E\u043B\u0430\u0434\u044B: ["\u0431\u043E\u043B\u0443"],
  \u0431\u043E\u043B\u0434\u044B: ["\u0431\u043E\u043B\u0443"],
  \u0431\u043E\u043B\u0493\u0430\u043D: ["\u0431\u043E\u043B\u0443"],
...
// src/data/dictionaries/kazakh_common_verbs.json
var kazakh_common_verbs_default = [
  "\u0430\u043B\u0443",
  "\u0430\u0439\u0442\u0443",
  "\u0430\u0448\u0443",
  "\u0430\u0443\u0440\u0443",
...
// src/data/dictionaries/kazakh_common_words.json
var kazakh_common_words_default = [
  "\u0430\u043B\u0443",
  "\u0430\u0439\u0442\u0443",
  "\u0430\u0448\u0443",
  "\u0430\u0443\u0440\u0443",
...
// src/data/dictionaries/polish_irregulars.json
var polish_irregulars_default = {
  jestem: ["by\u0107"],
  jeste\u015B: ["by\u0107"],
  jest: ["by\u0107"],
  jeste\u015Bmy: ["by\u0107"],
...
// src/data/dictionaries/polish_common_verbs.json
var polish_common_verbs_default = [
  "ba\u0107 si\u0119",
  "bra\u0107",
  "by\u0107",
  "chcie\u0107",
...
// src/data/dictionaries/swedish_irregulars.json
var swedish_irregulars_default = {
  \u00E4r: ["vara"],
  var: ["vara"],
  varit: ["vara"],
  vare: ["vara"],
...
// src/data/dictionaries/swedish_common_verbs.json
var swedish_common_verbs_default = [
  "anv\xE4nda",
  "arbeta",
  "be",
  "beh\xF6va",
...
// src/data/dictionaries/dutch_irregulars.json
var dutch_irregulars_default = {
  ben: ["zijn"],
  bent: ["zijn"],
  is: ["zijn"],
  zijn: ["zijn"],
...
// src/data/dictionaries/dutch_common_verbs.json
var dutch_common_verbs_default = [
  "antwoorden",
  "begrijpen",
  "beginnen",
  "bellen",
...
// src/data/dictionaries/japanese_irregulars.json
var japanese_irregulars_default = {
  \u3057\u307E\u3059: ["\u3059\u308B"],
  \u3057\u307E\u3057\u305F: ["\u3059\u308B"],
  \u3057\u306A\u3044: ["\u3059\u308B"],
  \u3057\u306A\u304B\u3063\u305F: ["\u3059\u308B"],
...
// src/data/dictionaries/japanese_common_verbs.json
var japanese_common_verbs_default = [
  "\u98DF\u3079\u308B",
  "\u305F\u3079\u308B",
  "\u98F2\u3080",
  "\u306E\u3080",
...
// src/data/dictionaries/chinese_irregulars.json
var chinese_irregulars_default = {
  \u770B\u4E86: ["\u770B"],
  \u770B\u7740: ["\u770B"],
  \u770B\u8457: ["\u770B"],
  \u770B\u8FC7: ["\u770B"],
...
// src/data/dictionaries/chinese_common_words.json
var chinese_common_words_default = [
  "\u770B",
  "\u53BB",
  "\u5403",
  "\u559D",
...
// src/data/dictionaries/arabic_irregulars.json
var arabic_irregulars_default = {
  \u0643\u0627\u0646: ["\u0643\u0627\u0646"],
  \u064A\u0643\u0648\u0646: ["\u0643\u0627\u0646"],
  \u062A\u0643\u0648\u0646: ["\u0643\u0627\u0646"],
  \u0623\u0643\u0648\u0646: ["\u0643\u0627\u0646"],
...
// src/data/dictionaries/arabic_common_words.json
var arabic_common_words_default = [
  "\u0643\u0627\u0646",
  "\u0642\u0627\u0644",
  "\u0631\u0623\u0649",
  "\u062C\u0627\u0621",
...
// src/data/dictionaries/korean_irregulars.json
var korean_irregulars_default = {
  \uAC00\uC694: ["\uAC00\uB2E4"],
  \uAC14\uC5B4\uC694: ["\uAC00\uB2E4"],
  \uAC11\uB2C8\uB2E4: ["\uAC00\uB2E4"],
  \uAC14\uC2B5\uB2C8\uB2E4: ["\uAC00\uB2E4"],
...
// src/data/dictionaries/korean_common_words.json
var korean_common_words_default = [
  "\uAC00\uB2E4",
  "\uBA39\uB2E4",
  "\uD558\uB2E4",
  "\uC788\uB2E4",
...
// src/data/dictionaries/turkish_irregulars.json
var turkish_irregulars_default = {
  gidiyor: ["gitmek"],
  gitti: ["gitmek"],
  gidecek: ["gitmek"],
  gider: ["gitmek"],
...
// src/data/dictionaries/turkish_common_verbs.json
var turkish_common_verbs_default = [
  "gelmek",
  "gitmek",
  "yapmak",
  "olmak",
...
// src/data/dictionaries/hindi_irregulars.json
var hindi_irregulars_default = {
  \u0939\u0942\u0901: ["\u0939\u094B\u0928\u093E"],
  \u0939\u0948: ["\u0939\u094B\u0928\u093E"],
  \u0939\u094B: ["\u0939\u094B\u0928\u093E"],
  \u0939\u0948\u0902: ["\u0939\u094B\u0928\u093E"],
...
// src/data/dictionaries/hindi_common_words.json
var hindi_common_words_default = [
  "\u0915\u0930\u0928\u093E",
  "\u0939\u094B\u0928\u093E",
  "\u091C\u093E\u0928\u093E",
  "\u0906\u0928\u093E",
...
// src/data/dictionaries/hebrew_irregulars.json
var hebrew_irregulars_default = {
  \u05D4\u05D9\u05D4: ["\u05DC\u05D4\u05D9\u05D5\u05EA"],
  \u05D4\u05D9\u05EA\u05D4: ["\u05DC\u05D4\u05D9\u05D5\u05EA"],
  \u05D4\u05D9\u05D5: ["\u05DC\u05D4\u05D9\u05D5\u05EA"],
  \u05D4\u05D9\u05D9\u05EA\u05D9: ["\u05DC\u05D4\u05D9\u05D5\u05EA"],
...
// src/data/dictionaries/hebrew_common_words.json
var hebrew_common_words_default = [
  "\u05DC\u05D4\u05D9\u05D5\u05EA",
  "\u05DC\u05D4\u05D2\u05D9\u05D3",
  "\u05DC\u05DC\u05DB\u05EA",
  "\u05DC\u05D1\u05D5\u05D0",
...
// src/data/dictionaries/greek_irregulars.json
var greek_irregulars_default = {
  \u03B5\u03AF\u03BC\u03B1\u03B9: ["\u03B5\u03AF\u03BC\u03B1\u03B9"],
  \u03B5\u03AF\u03C3\u03B1\u03B9: ["\u03B5\u03AF\u03BC\u03B1\u03B9"],
  \u03B5\u03AF\u03BD\u03B1\u03B9: ["\u03B5\u03AF\u03BC\u03B1\u03B9"],
  \u03B5\u03AF\u03BC\u03B1\u03C3\u03C4\u03B5: ["\u03B5\u03AF\u03BC\u03B1\u03B9"],
...
// src/data/dictionaries/greek_common_verbs.json
var greek_common_verbs_default = [
  "\u03B3\u03C1\u03AC\u03C6\u03C9",
  "\u03B4\u03B9\u03B1\u03B2\u03AC\u03B6\u03C9",
  "\u03B5\u03AF\u03BC\u03B1\u03B9",
  "\u03AD\u03C7\u03C9",
...
// src/data/dictionaries/finnish_irregulars.json
var finnish_irregulars_default = {
  olen: ["olla"],
  olet: ["olla"],
  on: ["olla"],
  olemme: ["olla"],
...
// src/data/dictionaries/finnish_common_verbs.json
var finnish_common_verbs_default = [
  "olla",
  "menn\xE4",
  "tulla",
  "n\xE4hd\xE4",
...
// src/data/dictionaries/hungarian_irregulars.json
var hungarian_irregulars_default = {
  vagyok: ["lenni", "van"],
  vagy: ["lenni", "van"],
  van: ["lenni", "van"],
  vagyunk: ["lenni", "van"],
...
// src/data/dictionaries/hungarian_common_verbs.json
var hungarian_common_verbs_default = [
  "lenni",
  "van",
  "menni",
  "j\xF6nni",
...
// src/data/dictionaries/czech_irregulars.json
var czech_irregulars_default = {
  jsem: ["b\xFDt"],
  jsi: ["b\xFDt"],
  je: ["b\xFDt"],
  jsme: ["b\xFDt"],
...
// src/data/dictionaries/czech_common_verbs.json
var czech_common_verbs_default = [
  "b\xFDt",
  "m\xEDt",
  "d\u011Blat",
  "j\xEDt",
...
// src/data/dictionaries/romanian_irregulars.json
var romanian_irregulars_default = {
  sunt: ["a fi", "fi"],
  e\u0219ti: ["a fi", "fi"],
  este: ["a fi", "fi"],
  e: ["a fi", "fi"],
...
// src/data/dictionaries/romanian_common_verbs.json
var romanian_common_verbs_default = [
  "a fi",
  "fi",
  "a avea",
  "avea",
...
// src/data/dictionaries/vietnamese_irregulars.json
var vietnamese_irregulars_default = {
  \u0111\u00E3: ["\u0111\xE3"],
  s\u1EBD: ["s\u1EBD"],
  \u0111ang: ["\u0111ang"],
  kh\u00F4ng: ["kh\xF4ng"],
...
// src/data/dictionaries/vietnamese_common_words.json
var vietnamese_common_words_default = [
  "\u0103n",
  "u\u1ED1ng",
  "\u0111i",
  "\u0111\u1EBFn",
...
// src/data/dictionaries/persian_irregulars.json
var persian_irregulars_default = {
  \u0647\u0633\u062A\u0645: ["\u0628\u0648\u062F\u0646"],
  \u0647\u0633\u062A\u06CC: ["\u0628\u0648\u062F\u0646"],
  \u0627\u0633\u062A: ["\u0628\u0648\u062F\u0646"],
  \u0647\u0633\u062A\u06CC\u0645: ["\u0628\u0648\u062F\u0646"],
...
// src/data/dictionaries/persian_common_words.json
var persian_common_words_default = [
  "\u0628\u0648\u062F\u0646",
  "\u062F\u0627\u0634\u062A\u0646",
  "\u0631\u0641\u062A\u0646",
  "\u0622\u0645\u062F\u0646",
...
// src/morphology.ts
var ENGLISH_IRREGULARS = english_irregulars_default;
var SPANISH_IRREGULARS = spanish_irregulars_default;
var PORTUGUESE_IRREGULARS = portuguese_irregulars_default;
var PORTUGUESE_COMMON_VERBS = new Set(portuguese_common_verbs_default);
var FRENCH_IRREGULARS = french_irregulars_default;
var FRENCH_COMMON_VERBS = new Set(french_common_verbs_default);
var ITALIAN_IRREGULARS = italian_irregulars_default;
var ITALIAN_COMMON_VERBS = new Set(italian_common_verbs_default);
var GERMAN_IRREGULARS = german_irregulars_default;
var GERMAN_COMMON_VERBS = new Set(german_common_verbs_default);
var RUSSIAN_IRREGULARS = russian_irregulars_default;
var RUSSIAN_COMMON_VERBS = new Set(russian_common_verbs_default);
var UKRAINIAN_IRREGULARS = ukrainian_irregulars_default;
var UKRAINIAN_COMMON_VERBS = new Set(ukrainian_common_verbs_default);
var KAZAKH_IRREGULARS = kazakh_irregulars_default;
var KAZAKH_COMMON_VERBS = new Set(kazakh_common_verbs_default);
var KAZAKH_COMMON_WORDS = /* @__PURE__ */ new Set([
  ...kazakh_common_verbs_default,
  ...kazakh_common_verbs_default.map((s3) => s3.toLowerCase()),
  ...kazakh_common_words_default,
  ...kazakh_common_words_default.map((s3) => s3.toLowerCase())
]);
var POLISH_IRREGULARS = polish_irregulars_default;
var POLISH_COMMON_VERBS = new Set(polish_common_verbs_default);
var SWEDISH_IRREGULARS = swedish_irregulars_default;
var SWEDISH_COMMON_VERBS = new Set(swedish_common_verbs_default);
var DUTCH_IRREGULARS = dutch_irregulars_default;
var DUTCH_COMMON_VERBS = new Set(dutch_common_verbs_default);
var JAPANESE_IRREGULARS = japanese_irregulars_default;
var JAPANESE_COMMON_VERBS = new Set(japanese_common_verbs_default);
var CHINESE_IRREGULARS = chinese_irregulars_default;
var CHINESE_COMMON_WORDS = new Set(chinese_common_words_default);
var ARABIC_IRREGULARS = arabic_irregulars_default;
var ARABIC_COMMON_WORDS = new Set(arabic_common_words_default);
var KOREAN_IRREGULARS = korean_irregulars_default;
var KOREAN_COMMON_WORDS = new Set(korean_common_words_default);
var TURKISH_IRREGULARS = turkish_irregulars_default;
var TURKISH_COMMON_VERBS = new Set(turkish_common_verbs_default);
var HINDI_IRREGULARS = hindi_irregulars_default;
var HINDI_COMMON_WORDS = new Set(hindi_common_words_default);
var HEBREW_IRREGULARS = hebrew_irregulars_default;
var HEBREW_COMMON_WORDS = new Set(hebrew_common_words_default);
var GREEK_IRREGULARS = greek_irregulars_default;
var GREEK_COMMON_VERBS = new Set(greek_common_verbs_default);
var FINNISH_IRREGULARS = finnish_irregulars_default;
var FINNISH_COMMON_VERBS = new Set(finnish_common_verbs_default);
var HUNGARIAN_IRREGULARS = hungarian_irregulars_default;
var HUNGARIAN_COMMON_VERBS = new Set(hungarian_common_verbs_default);
var CZECH_IRREGULARS = czech_irregulars_default;
var CZECH_COMMON_VERBS = new Set(czech_common_verbs_default);
var ROMANIAN_IRREGULARS = romanian_irregulars_default;
var ROMANIAN_COMMON_VERBS = new Set(romanian_common_verbs_default);
var VIETNAMESE_IRREGULARS = vietnamese_irregulars_default;
var VIETNAMESE_COMMON_WORDS = new Set(vietnamese_common_words_default);
var PERSIAN_IRREGULARS = persian_irregulars_default;
var PERSIAN_COMMON_WORDS = new Set(persian_common_words_default);
var SPANISH_DOUBLE_ENCLITICS = spanish_enclitics_default.double;
var SPANISH_SINGLE_ENCLITICS = spanish_enclitics_default.single;
var SPANISH_SHORT_IMPERATIVES = spanish_short_imperatives_default;
var SPANISH_FUTURE_STEMS = spanish_future_stems_default;
var SPANISH_HIGH_CONFIDENCE_ENDINGS = new Set(spanish_high_confidence_endings_default);
var ACCENTED_VERBS_MAP = spanish_accented_verbs_default;
var SPANISH_VERB_ENDINGS = spanish_verb_endings_default;
var SPANISH_COMMON_VERBS = new Set(spanish_common_verbs_default);
var SPANISH_HOMONYM_PRIORITY = spanish_homonyms_default.priority;
var SPANISH_HOMONYM_EXCLUDE = new Set(spanish_homonyms_default.exclude);
function normalizeAccents(str) {
  return str.replace(/á/g, "a").replace(/é/g, "e").replace(/í/g, "i").replace(/ó/g, "o").replace(/ú/g, "u").replace(/ü/g, "u");
}
function stripSpanishEnclitics(w) {
  const candidates = [];
  const hasAccent = /[áéíóú]/.test(w);
  const checkValidBase = (stripped) => {
    ...
    const shortBases = /* @__PURE__ */ new Set(["di", "da", "haz", "pon", "ten", "ve", "sal", "ven", "val", "trae", "oye", "se", "s\xE9"]);
...
var SPANISH_PAST_GERUND_ENDINGS = /* @__PURE__ */ new Set([
  "iendo",
  "yendo",
  "i\xF3",
  "ieron",
...
function getSpanishStemVariations(stem, ending) {
  const variations = [stem];
  if (stem.includes("ie")) {
    variations.push(stem.replace("ie", "e"));
    variations.push(stem.replace("ie", "i"));
  }
  if (stem.includes("ue")) {
  ...
  const finalVars = /* @__PURE__ */ new Set();
  for (const v2 of variations) {
    finalVars.add(v2);
    if (v2.endsWith("aig") || v2.endsWith("oig")) {
      finalVars.add(v2.slice(0, -2));
    } else if (v2.endsWith("g")) {
      finalVars.add(v2.slice(0, -1));
    }
    if (v2.endsWith("qu")) {
      finalVars.add(v2.slice(0, -2) + "c");
    }
    if (v2.endsWith("gu")) {
      finalVars.add(v2.slice(0, -2) + "g");
    }
    if (v2.endsWith("z")) {
      finalVars.add(v2.slice(0, -1) + "c");
    }
    if (v2.endsWith("c")) {
      finalVars.add(v2.slice(0, -1) + "z");
    }
    if (v2.endsWith("g")) {
      finalVars.add(v2 + "u");
    }
    if (v2.endsWith("uy")) {
      finalVars.add(v2.slice(0, -2) + "u");
    }
  }
  return Array.from(finalVars);
}
function getPortugueseEncliticCandidates(word) {
  ...
  if (nextPart && (nextPart === "lo" || nextPart === "la" || nextPart === "los" || nextPart === "las")) {
    ...
    const unaccented = verbPart.replace(/[áàâã]/g, "a").replace(/[éê]/g, "e").replace(/[íî]/g, "i").replace(/[óôõ]/g, "o").replace(/[úû]/g, "u");
    candidates.push(unaccented + "r");
    if (verbPart === "p\xF4") candidates.push("p\xF4r");
    if (verbPart === "qu\xEA") candidates.push("querer");
    if (verbPart === "di") candidates.push("dizer");
    if (verbPart === "fa") candidates.push("fazer");
  }
  return candidates;
}
function restoreSpanishAccents(suggestion, original) {
  const originalLower = original.toLowerCase();
  const sugLower = suggestion.toLowerCase();
  const pluralsMap = {
    "an": { plurSuffix: "anes", singSuffix: "an", accentedSing: "\xE1n" },
    "en": { plurSuffix: "enes", singSuffix: "en", accentedSing: "\xE9n" },
    "in": { plurSuffix: "ines", singSuffix: "in", accentedSing: "\xEDn" },
    "on": { plurSuffix: "ones", singSuffix: "on", accentedSing: "\xF3n" },
    "un": { plurSuffix: "unes", singSuffix: "un", accentedSing: "\xFAn" }
  };
  ...
  let accentedChar = null;
  let accentedIndex = -1;
  for (let i3 = 0; i3 < originalLower.length; i3++) {
    const char = originalLower[i3];
    if (accentMap[char]) {
      accentedChar = char;
      accentedIndex = i3;
      break;
  ...
  let restored = "";
  for (let i3 = 0; i3 < suggestion.length; i3++) {
    const sugChar = suggestion[i3];
    const sugCharLower = sugChar.toLowerCase();
    if (i3 === accentedIndex && sugCharLower === cleanOrigAccented) {
  ...
  if ((cleanRestoredLower.endsWith("ico") || cleanRestoredLower.endsWith("ica")) && !/[áéíóú]/.test(restored)) {
    let vowelIndex = -1;
    for (let i3 = restored.length - 4; i3 >= 0; i3--) {
      const charLower = restored[i3].toLowerCase();
...
function addFrenchStemVariations(stem, verbSuggestions) {
  if (!stem || stem.length < 2) return;
  verbSuggestions.push(stem + "er");
  verbSuggestions.push(stem + "ir");
  verbSuggestions.push(stem + "re");
  if (stem.endsWith("\xE7")) verbSuggestions.push(stem.slice(0, -1) + "cer");
  if (stem.endsWith("ge")) verbSuggestions.push(stem.slice(0, -2) + "ger");
  if (stem.endsWith("\xE8")) verbSuggestions.push(stem.slice(0, -1) + "er");
...
function addItalianStemVariations(stem, verbSuggestions) {
  if (!stem || stem.length < 2) return;
...
function stripItalianEnclitics(word) {
  const enclitics = [
    "melo",
    "mela",
    "meli",
...
function addPolishStemVariations(stem, verbSuggestions, isReflexive = false) {
  if (!stem || stem.length < 2) return;
  const pushInfinitive = (inf) => {
...
function addSwedishStemVariations(stem, verbSuggestions) {
  if (!stem || stem.length < 2) return;
  verbSuggestions.push(stem + "a");
  verbSuggestions.push(stem);
}
function addDutchStemVariations(stem, verbSuggestions) {
  if (!stem || stem.length < 2) return;
...
function normalizeArabic(str) {
  if (!str) return "";
  return str.replace(/[\u064B-\u0652\u0670]/g, "").replace(/[أإآ]/g, "\u0627").replace(/ى/g, "\u064A").replace(/ؤ/g, "\u0648").replace(/ئ/g, "\u064A");
}
...
function normalizePersian(str) {
  if (!str) return "";
  return str.replace(/[\u064B-\u0652]/g, "").replace(/[\u064A\u0649]/g, "\u06CC").replace(/\u0643/g, "\u06A9").replace(/\u200C/g, "");
}
function stripGreekAccents(str) {
  if (!str) return "";
  return str.replace(/[άὰάἄἄἂἂἆἆἁἁἃἃἇἇ]/g, "\u03B1").replace(/[έὲέἔἔἓἓ]/g, "\u03B5").replace(/[ήὴήἤἤἢἢἦἦἡἡἣἣἧἧ]/g, "\u03B7").replace(/[ίὶίἴἴἲἲἶἶἱἱἳἳἷἷϊΐ]/g, "\u03B9").replace(/[όὸόὄὄὃὃὁὁὃὃ]/g, "\u03BF").replace(/[ύὺύὔὔὒὒὖὖὑὑὓὓὗὗϋΰ]/g, "\u03C5").replace(/[ώὼώὤὤὢὢὦὦὡὡὣὣὧὧ]/g, "\u03C9").replace(/ς/g, "\u03C3");
}
function getSuggestedLemmas(word, targetLanguage) {
  if (!word) return [];
  ...
  if (lang.startsWith("en") || lang === "\u0430\u043D\u0433\u043B\u0438\u0439\u0441\u043A\u0438\u0439" || lang === "english") {
    ...
    try {
      const doc = three_default(w);
      doc.compute("root");
      const root = doc.json()[0]?.terms?.[0]?.root || "";
    ...
    if (suggestions.length === 0) {
      if (w.endsWith("ed") && w.length > 3) {
        ...
        suggestions.push(w.slice(0, -3) + "e");
      }
    }
  } else if (lang.startsWith("es") || lang.startsWith("spa") || lang === "\u0438\u0441\u043F\u0430\u043D\u0441\u043A\u0438\u0439" || lang === "spanish") {
    const verbSuggestions = [];
    const nounAdjSuggestions = [];
    ...
    for (let cand of Array.from(new Set(candidates))) {
      ...
      if (cand.endsWith("as") && cand.length > 3) {
        ...
        nounAdjSuggestions.push(cand.slice(0, -3) + "z");
      ...
      for (const data of matchedRulesData) {
        if (data.foundIrregular) {
          ...
          for (const v2 of SPANISH_FUTURE_STEMS[stem]) {
            pushVerbSuggestion(v2);
            foundCommonVerb = true;
          }
        }
        const validVerbs = data.generatedVerbs.filter((v2) => SPANISH_COMMON_VERBS.has(v2));
        if (validVerbs.length > 0) {
          foundCommonVerb = true;
          for (const v2 of validVerbs) {
            pushVerbSuggestion(v2);
          }
        }
      }
    ...
    suggestions.push(...nounAdjSuggestions.map((s3) => restoreSpanishAccents(s3, w)));
    ...
    for (let cand of Array.from(new Set(candidates))) {
      ...
      if (cand.endsWith("ar") || cand.endsWith("er") || cand.endsWith("ir") || cand.endsWith("or") || cand.endsWith("\xF4r")) {
        verbSuggestions.push(cand);
      }
      ...
      if (cand.endsWith("s") && cand.length > 2) {
        if (cand.endsWith("\xF5es") || cand.endsWith("\xE3es") || cand.endsWith("\xE3os")) {
          nounAdjSuggestions.push(cand.slice(0, -3) + "\xE3o");
      ...
      if (cand.endsWith("ava") || cand.endsWith("avam") || cand.endsWith("avas") || cand.endsWith("\xE1vamos")) {
        verbSuggestions.push(cand.replace(/á|a(va|vam|vas|vamos)$/, "") + "ar");
      }
      if (cand.endsWith("ia") || cand.endsWith("iam") || cand.endsWith("ias") || cand.endsWith("\xEDamos")) {
    ...
    const validVerbs = Array.from(new Set(verbSuggestions)).filter((v2) => PORTUGUESE_COMMON_VERBS.has(v2));
    if (validVerbs.length > 0) {
    ...
    if (FRENCH_IRREGULARS[w] || FRENCH_IRREGULARS[baseWord] || FRENCH_IRREGULARS[normWord]) {
      ...
      return Array.from(new Set(verbSuggestions)).filter((s3) => s3 !== w);
    }
    ...
    } else if (baseWord.endsWith("\xE9") || baseWord.endsWith("\xE9s") || baseWord.endsWith("\xE9e") || baseWord.endsWith("\xE9es")) {
    ...
    const validVerbs = Array.from(new Set(verbSuggestions)).filter((v2) => FRENCH_COMMON_VERBS.has(v2));
    if (validVerbs.length > 0) {
      suggestions.push(...validVerbs);
    } else {
      suggestions.push(...verbSuggestions);
    ...
    const validVerbs = Array.from(new Set(verbSuggestions)).filter((v2) => ITALIAN_COMMON_VERBS.has(v2));
    if (validVerbs.length > 0) {
      suggestions.push(...validVerbs);
    } else {
      suggestions.push(...verbSuggestions);
    ...
    if (GERMAN_IRREGULARS[w] || GERMAN_IRREGULARS[normWord]) {
      ...
      return Array.from(new Set(suggestions)).filter((s3) => s3 !== w);
    }
    ...
    if (baseWord.endsWith("ami") || baseWord.endsWith("ach") || baseWord.endsWith("om") || baseWord.endsWith("\xF3w")) {
    ...
    const validVerbs = Array.from(new Set(verbSuggestions)).filter((v2) => POLISH_COMMON_VERBS.has(v2));
    if (validVerbs.length > 0) {
      suggestions.push(...validVerbs);
    } else {
      suggestions.push(...verbSuggestions);
    ...
    const validVerbs = Array.from(new Set(verbSuggestions)).filter((v2) => SWEDISH_COMMON_VERBS.has(v2));
    if (validVerbs.length > 0) {
      suggestions.push(...validVerbs);
    } else {
      suggestions.push(...verbSuggestions);
    ...
    if (w.includes("ge") && (w.endsWith("d") || w.endsWith("t") || w.endsWith("en")) && w.length > 4) {
      ...
      const prefix5 = w.substring(0, idx);
    ...
    const validVerbs = Array.from(new Set(verbSuggestions)).filter((v2) => DUTCH_COMMON_VERBS.has(v2));
    if (validVerbs.length > 0) {
      suggestions.push(...validVerbs);
    } else {
      suggestions.push(...verbSuggestions);
    ...
    const validWords = Array.from(new Set(wordSuggestions)).filter((v2) => CHINESE_COMMON_WORDS.has(v2));
    ...
    const candidateBases = /* @__PURE__ */ new Set([w, normW]);
    const prefixes2 = [
      "\u0648\u0628\u0627\u0644",
      "\u0648\u0641\u0627\u0644",
      "\u0648\u0643\u0627\u0644",
    ...
    const validWords = Array.from(new Set(wordSuggestions)).filter((v2) => ARABIC_COMMON_WORDS.has(v2) || ARABIC_COMMON_WORDS.has(normalizeArabic(v2)));
    if (validWords.length > 0) {
      suggestions.push(...validWords);
    }
    ...
    const validWords = Array.from(new Set(wordSuggestions)).filter((v2) => KOREAN_COMMON_WORDS.has(v2));
    if (validWords.length > 0) {
      suggestions.push(...validWords);
    }
  } else if (lang.startsWith("tr") || lang.startsWith("tur") || lang === "\u0442\u0443\u0440\u0435\u0446\u043A\u0438\u0439" || lang === "t\xFCrk\xE7e" || lang === "turkish") {
    const rawWord = word.trim();
    ...
    const validWords = Array.from(new Set(wordSuggestions)).filter((v2) => HINDI_COMMON_WORDS.has(v2));
    if (validWords.length > 0) {
      suggestions.push(...validWords);
    }
    ...
    const visited = /* @__PURE__ */ new Set([w, normW]);
    ...
    const validWords = Array.from(new Set(wordSuggestions)).filter((v2) => HEBREW_COMMON_WORDS.has(v2) || HEBREW_COMMON_WORDS.has(normalizeHebrew(v2)));
    if (validWords.length > 0) {
      suggestions.push(...validWords);
    }
    ...
    const validWords = Array.from(new Set(wordSuggestions)).filter((v2) => GREEK_COMMON_VERBS.has(v2) || GREEK_COMMON_VERBS.has(stripGreekAccents(v2)));
    if (validWords.length > 0) {
      suggestions.push(...validWords);
    }
    ...
    const validWords = Array.from(new Set(wordSuggestions)).filter((v2) => CZECH_COMMON_VERBS.has(v2));
    if (validWords.length > 0) {
      suggestions.push(...validWords);
    }
  } else if (lang.startsWith("ro") || lang.startsWith("rum") || lang.startsWith("ron") || lang === "\u0440\u0443\u043C\u044B\u043D\u0441\u043A\u0438\u0439" || lang === "rom\xE2n\u0103" || lang === "romanian") {
    const wordSuggestions = [];
    let base = w;
    ...
    const validWords = Array.from(new Set(wordSuggestions)).filter((v2) => ROMANIAN_COMMON_VERBS.has(v2));
    if (validWords.length > 0) {
      suggestions.push(...validWords);
    }
    ...
    const validWords = Array.from(new Set(wordSuggestions)).filter((v2) => PERSIAN_COMMON_WORDS.has(v2) || PERSIAN_COMMON_WORDS.has(normalizePersian(v2)));
    if (validWords.length > 0) {
      suggestions.push(...validWords);
    }
  } else {
  ...
  let unique2 = Array.from(new Set(suggestions)).map((s3) => s3.trim()).filter((s3) => s3.length > 1);
  if (unique2.includes(w)) {
    const withoutW = unique2.filter((s3) => s3 !== w);
  ...
  if (lang.startsWith("es") || lang.startsWith("spa") || lang === "\u0438\u0441\u043F\u0430\u043D\u0441\u043A\u0438\u0439" || lang === "spanish") {
    ...
    if (SPANISH_HOMONYM_PRIORITY[w]) {
      const prioritized = SPANISH_HOMONYM_PRIORITY[w].filter((p5) => unique2.includes(p5));
      const rest = unique2.filter((u2) => !SPANISH_HOMONYM_PRIORITY[w].includes(u2));
    }
  }
  return filterValidLemmas(unique2);
}
function filterValidLemmas(suggestions) {
  return suggestions.filter((s3) => {
    if (!s3 || s3.length < 2) return false;
    if (/[áéíóú].*(?:ir|er|ar)$/i.test(s3)) return false;
    if (/(?:ir|er|ar)(?:ir|er|ar)$/i.test(s3)) return false;
    return true;
  });
}

// extension/src/background/service-worker.ts
var apiClient = new LecturaApiClient();
...
chrome.runtime.onInstalled.addListener(async () => {
  ...
  chrome.contextMenus.create({
    ...
    title: '\u{1F4BE} Save Word "%s" to Lectura',
    contexts: ["selection"]
  });
...
async function translateDirectInWorker(text, sourceLang = "es", targetLang = "ru") {
  ...
  const isMultiWord = clean2.includes(" ");
  const dtParams = isMultiWord ? "dt=t" : "dt=t&dt=bd&dt=rm";
  try {
    const url = `https://translate.googleapis.com/translate_a/single?client=gtx&sl=${sIso}&tl=${tIso}&${dtParams}&q=${encodeURIComponent(clean2)}`;
    ...
    if (resp.ok) {
      ...
      if (data) {
        if (data[1] && Array.isArray(data[1]) && data[1].length > 0) {
          ...
          for (const posBlock of data[1]) {
            if (posBlock && typeof posBlock[0] === "string" && Array.isArray(posBlock[1]) && posBlock[1].length > 0) {
              ...
              const topWords = posBlock[1].slice(0, 4).map((w) => String(w).trim()).filter(Boolean);
        ...
        if (Array.isArray(data[0])) {
          const primary = data[0].map((item) => item && item[0] ? item[0] : "").join("").trim();
        ...
        if (data[5] && Array.isArray(data[5]) && data[5][0] && Array.isArray(data[5][0][2])) {
          for (const item of data[5][0][2]) {
            if (item && typeof item[0] === "string" && item[0].trim()) {
              ...
              if (!definitions.some((d2) => d2.toLowerCase() === cleanSyn.toLowerCase())) {
...
chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
  if (message.type === "CHECK_HEALTH") {
    apiClient.checkHealth().then((res) => sendResponse({ success: true, data: res })).catch((err) => sendResponse({ success: false, error: err.message }));
    return true;
  }
  if (message.type === "SAVE_LESSON") {
    apiClient.saveLesson(message.payload).then((res) => sendResponse({ success: true, data: res })).catch((err) => sendResponse({ success: false, error: err.message }));
    return true;
  }
  if (message.type === "SAVE_WORD") {
    apiClient.saveWord(message.payload).then((res) => sendResponse({ success: true, data: res })).catch((err) => sendResponse({ success: false, error: err.message }));
    return true;
  }
  if (message.type === "GET_WORDS") {
    const { language } = message.payload || {};
    apiClient.getWords(language).then((res) => sendResponse({ success: true, data: res })).catch((err) => sendResponse({ success: false, error: err.message }));
    return true;
  }
  if (message.type === "BATCH_WORD_STATUS") {
    const { words, language } = message.payload || {};
    apiClient.getBatchWordStatuses(words, language).then((res) => sendResponse({ success: true, data: res })).catch((err) => sendResponse({ success: false, error: err.message }));
    return true;
  }
  if (message.type === "GET_PROFILES") {
    apiClient.getProfiles().then((res) => sendResponse({ success: true, data: res })).catch((err) => sendResponse({ success: false, error: err.message }));
    return true;
  }
  if (message.type === "TRANSLATE_TEXT" || message.type === "TRANSLATE_WORD") {
    const text = (message.payload?.text || message.payload?.word || message.text || message.word || "").trim();
    const sourceLang = message.payload?.sourceLang || message.sourceLang || "auto";
    const targetLang = message.payload?.targetLang || message.targetLang || "ru";
    (async () => {
      console.log("[Lectura ServiceWorker] Translation requested:", { text, sourceLang, targetLang });
      try {
        const timeoutPromise = new Promise(
          (_2, reject) => setTimeout(() => reject(new Error("Server timeout (1.8s)")), 1800)
        );
      ...
      try {
        ...
        console.error("[Lectura ServiceWorker] Fallback translation failed:", e2?.message || e2);
  ...
  if (message.type === "SAVE_WORD_LINK") {
    apiClient.saveWordLink(message.payload).then((res) => sendResponse({ success: true, data: res })).catch((err) => sendResponse({ success: false, error: err.message }));
    return true;
  }
  if (message.type === "GET_WORD_LINKS") {
    const { language } = message.payload || {};
    apiClient.getWordLinks(language).then((res) => sendResponse({ success: true, data: { links: res } })).catch((err) => sendResponse({ success: false, error: err.message }));
    return true;
  }
  if (message.type === "LEMMATIZE_WORD") {
    const { word, language } = message.payload || {};
    apiClient.lemmatizeWord(word, language).then((res) => sendResponse({ success: true, data: res })).catch((err) => sendResponse({ success: false, error: err.message }));
    return true;
  }
  ...
  if (message.type === "TRANSLATE_WORD") {
    ...
    const tIso = (targetLang || "ru").slice(0, 2).toLowerCase();
    const clean2 = cleanWordForTranslation(word || "");
    const isMultiWord = clean2.includes(" ");
    const dtParams = isMultiWord ? "dt=t" : "dt=t&dt=bd&dt=rm";
    const url = `https://translate.googleapis.com/translate_a/single?client=gtx&sl=${sIso}&tl=${tIso}&${dtParams}&q=${encodeURIComponent(clean2)}`;
    fetch(url).then((res) => {
        ...
        for (const entry of data[1]) {
          if (entry && Array.isArray(entry[1])) {
            for (const term of entry[1]) {
              ...
              if (cleanTerm && cleanTerm.toLowerCase() !== word.toLowerCase() && !variants.some((v2) => v2.toLowerCase() === cleanTerm.toLowerCase())) {
      ...
      if (primary && primary.toLowerCase() !== word.toLowerCase() && !variants.some((v2) => v2.toLowerCase() === primary.toLowerCase())) {
  ...
  if (message.type === "LINK_WORD_ROOT") {
    ...
    apiClient.saveWordLink({ wordFrom, wordTo, language }).then((res) => sendResponse({ success: true, data: res })).catch((err) => sendResponse({ success: false, error: err.message }));
    return true;
  }
